...
def x_y_tvt(train, validate, test, target):
    '''
    This function will create subsets for train, validate, and test for modeling
    '''  
    seed = 42
...
def rfe(n_features, X_train, y_train):
    '''
    This function will take in a number of features, an X_train, and a y_train, 
    to determine which features (based on the number chosen) are best suited to 
    modeling for the best RMSE based on the rfe method.
    '''
    lm = LinearRegression()
...
def f_selector(k, X_train, y_train):
    '''
    This function will take in a number, an X_train, and a y_train, to determine which 
    features (based on the number chosen) are best suited to modeling for the best RMSE, 
    based upon the SelectKBest method.
    '''
...
def create_preds_df(y_train):
    '''
    This function will create a dataframe, 'preds_df', using the y_train (or validate or test), 
    and create a baseline column (using median), and return the dataframe.
    '''
...
def lin_regression(X_train, y_train, preds_df):
    '''
    This function will take in an X_train, y_train, and a preds_df (or validate or test), 
    and create, fit and predict upon a simple Linearregression() model, add the predictions to the 
    preds_df, and return it.
    '''
    lm = LinearRegression()
...
def lasso_lars(X_train, y_train, preds_df, alpha=.1):
    '''
    This function will take in an X_train, y_train, and a preds_df (or validate or test) 
    and an alpha (with a default of 0.1), and create, fit and predict upon a simple LassoLars() 
    model, add the predictions to the preds_df, and return it. 
    '''
...
def glm_model(X_train, y_train, preds_df, power=0):
    '''
    This function will take in an X_train, y_train, a preds_df, and a power (with a default of 0), 
    and create, fit, and predict upon the TweedieRegressor() (GLM) model, add those predictions to a 
    preds_df, and return it.
    '''
...
def poly_subset(X_train, y_train, degree=2):
    '''
    This function will take in an X_train, a y_train, as well as a number for degrees, 
    to create a polynomial subset of the data, fitting and transforming it 
    for use in other regression models
    '''
...
def poly_model(X_polynomial, y_train, preds_df, m):
    '''
    This function will take in a polynomial subset, a y_train, the preds_df, and a model type (m), 
    to create, fit, and predict on the model, as well as creating a column for it in the preds_df
    '''
    model = m
...
def get_rmses(preds_df):
    '''
    This function will take in the preds_df andcreate another dataframe with the 
